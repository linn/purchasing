namespace Linn.Purchasing.Domain.LinnApps.Reports
{
    ...
    public class BomHistoryReportService : IBomHistoryReportService
    {
        ...
        {
            var subAssemblies = this.treeService.FlattenBomTree(bomName, null, false).Where(x => x.Type != "C")
                .Select(x => x.Name);
                ...
                g => new BomHistoryReportLine
                         {
                             ChangeId = g.Key,
                             DetailId =
                                 $"{g.First().DetailId}{(g.Count() > 1 ? Environment.NewLine + g.ElementAt(1).DetailId : string.Empty)}",
                             DocumentNumber = g.First().DocumentNumber,
                             BomName = g.First().BomName,
                             DateApplied = g.First().DateApplied,
                             AppliedBy = g.First().AppliedBy,
                             DocumentType = g.First().DocumentType,
                             Operation =
                                 $"{g.First().Operation}{(g.Count() > 1 ? Environment.NewLine + g.ElementAt(1).Operation : string.Empty)}",
                             PartNumber =
                                 $"{g.First().PartNumber}{(g.Count() > 1 ? Environment.NewLine + g.ElementAt(1).PartNumber : string.Empty)}",
                             Qty =
                                 $"{g.First().Qty}{(g.Count() > 1 ? Environment.NewLine + g.ElementAt(1).Qty : string.Empty)}",
                             GenerateRequirement =
                                 $"{g.First().GenerateRequirement}{(g.Count() > 1 ? Environment.NewLine + g.ElementAt(1).GenerateRequirement : string.Empty)}"
                         });
        }
    }
}

namespace Linn.Purchasing.Domain.LinnApps.Tests.DeliveryPerformanceReportServiceTests
{
    ...
    public class WhenGettingPerformanceSummary : ContextBase
    {
        ...
        public void ShouldReturnReport()
        {
            ...
            this.result.GetGridValue(0, 1).Should().Be(15);
            this.result.GetGridValue(0, 2).Should().Be(13);
            this.result.GetGridValue(0, 3).Should().Be(86.7m);
            this.result.GetGridValue(0, 4).Should().Be(0);
            this.result.GetGridValue(0, 5).Should().Be(2);
            this.result.GetGridValue(0, 6).Should().Be(0);

            this.result.GetGridValue(1, 1).Should().Be(30);
            this.result.GetGridValue(1, 2).Should().Be(24);
            this.result.GetGridValue(1, 3).Should().Be(80.0m);
            this.result.GetGridValue(1, 4).Should().Be(2);
            this.result.GetGridValue(1, 5).Should().Be(2);
            this.result.GetGridValue(1, 6).Should().Be(2);

            this.result.GetTotalValue(1).Should().Be(45);
            this.result.GetTotalValue(2).Should().Be(37);
            this.result.GetTotalValue(3).Should().Be(82.2m);
            this.result.GetTotalValue(4).Should().Be(2);
            this.result.GetTotalValue(5).Should().Be(4);

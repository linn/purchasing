namespace Linn.Purchasing.Domain.LinnApps.Tests.PurchaseOrderReqServiceTests
{
    ...
    public class WhenAuthorisingWrongState : ContextBase
    {
        ...
        public void ShouldThrowUnauthorisedException()
        {
            this.action.Should().Throw<UnauthorisedActionException>()
                .WithMessage("Cannot authorise a req that is not in state 'AUTHORISE WAIT'. Please make sure the req is saved in this state and try again");
        }
    }
}
